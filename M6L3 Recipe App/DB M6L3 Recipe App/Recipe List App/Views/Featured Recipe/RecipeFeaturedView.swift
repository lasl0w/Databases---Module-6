...
//  Recipe List App
//
//  Created by Tom Montgomery on 06/29/23
//

import SwiftUI

struct RecipeFeaturedView: View {
    // coreData refactor - could get rid of the viewContext here
    @Environment(\.managedObjectContext) private var viewContext
    
    // no longer need EO now that we are fetching from core data
    //@EnvironmentObject var model:RecipeModel
    @State var isDetailViewShowing = false
    @State var tabSelectionIndex = 0
    
    // Refactor to core date instead of model.recipes
    // minimum/simple version is just sortDescriptors (and they can be an empty array)
    // MUST specify the data type we want back from coredata <Recipe>
    //@FetchRequest(sortDescriptors: [NSSortDescriptor(key: "name", ascending: true)]) var recipes: FetchedResults<Recipe>
    // Optimization #1:  default sort is reverse alpha?  odd.  set a sortDescriptor.  'ascending' = A to Z
    ...
    // Optimization #2: Fetch only what we need.  Add a predicate
            ...
            GeometryReader { geo in
                ...
                TabView (selection: $tabSelectionIndex) {
                    ...
                    ForEach (0..<recipes.count) { index in
                        ...
                        // Coredata refactor - no longer need b/c we are pulling back only featured recipes
